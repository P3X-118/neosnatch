...
pub fn detect() -> Option<ShellInfo> {
    ...
    Some(ShellInfo { name, path, version })
}

fn user_shell_from_passwd() -> Option<String> {
    let uid = nix::unistd::getuid();
    nix::unistd::User::from_uid(uid).ok().flatten()
...
fn probe_version(name: &str, path: &str) -> Option<String> {
    ...
    let (args, parser): (&[&str], fn(&str) -> Option<String>) = match name {
...
fn parse_fish(s: &str) -> Option<String> {
    // "fish, version 3.7.1"
    s.split("version ").nth(1)?.split_whitespace().next().map(str::to_owned)
}
