...
async fn main() -> Result<()> {
    ...
    if args.ports {
        return print_ports_full(&facts);
    }
    if args.cron {
        return print_cron_full(&facts);
    }
...
fn print_ports_full(facts: &collect::Facts) -> Result<()> {
    use std::net::IpAddr;
    println!("{:<6}  {:>5}  service", "scope", "port");
    let mut rows: Vec<(bool, u16, String)> = facts
        .listening_ports
        .iter()
        .map(|l| {
...
fn print_cron_full(facts: &collect::Facts) -> Result<()> {
    ...
    for c in &facts.cron_jobs {
        println!(
            "{:<32}  {:<14}  {:<12}  {}",
            c.source, c.schedule, c.user, c.command
        );
    }
    Ok(())
}
